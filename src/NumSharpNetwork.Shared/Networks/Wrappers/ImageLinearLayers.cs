...
namespace NumSharpNetwork.Shared.Networks.Wrappers
{
    public class ImageLinearLayers : NetworkWrapper
    {
        ...
        public ImageLinearLayers(int height, int width, int channels, int numClasses, IOptimizer optimizer)
        {
            FlattenLayer flatten = new FlattenLayer();

            int linearInputSize = (height) * (width) * channels;

            LinearLayer linear1 = new LinearLayer(optimizer, linearInputSize, 100, name: $"{this.Name}.LinearLayer1");

            ReLULayer reLU1 = new ReLULayer();

            LinearLayer linear2 = new LinearLayer(optimizer, 100, numClasses, name: $"{this.Name}.LinearLayer2");
